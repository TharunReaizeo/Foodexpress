...
import com.foodexpress.dto.MenuItems;
...
public class MenuDAO {
    ...
    public boolean addMenuItem(MenuItems item) {
        String sql = "INSERT INTO menu_item (restaurant_id, item_name, description, price, category, is_veg, rating) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  //Search by category like starters,main course,dessert etc...,
    public List<MenuItems> filterByCategory(String category) {
        List<MenuItems> itemList = new ArrayList<>();

        String sql = "SELECT * FROM menu_item WHERE category = ?";
 ...
 //Sorting the Items by price and rating either ASC or DESC   
    public List<MenuItems> sortItems(String criteria) {
        ...
        String sql = "SELECT * FROM menu_item ORDER BY " + orderBy;
    ...
    //Filter by Only Veg (PURE VEG)
    public List<MenuItems> filterVegOrNonVeg(String vegOrNonVeg){
    	List<MenuItems> itemList = new ArrayList<>();
    	
    	String sql="Select * from Menuitems where type = ?";
    	try(PreparedStatement ps = DBConnection.giveConnection().prepareStatement(sql)) {
    		
    		 ps.setString(1, vegOrNonVeg);
   ...
   //search Food item by city
    
    
//========================================================================================================
    
 // Helper Method to map ResultSet to MenuItem object
